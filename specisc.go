...
// NextInactiveWithAlloc returns the next time this schedule is deactivated, greater than the given
// time.  If the schedule is always active in the next 5 years, return the zero time.
func (s *SpecSchedule) NextInactiveWithAlloc(t time.Time) time.Time {
	// General approach
	//
	// This implementation is the mirror of the original Next implementation. Only the comparison operator is changed.
	// For Month, Day, Hour, Minute, Second:
	...
	if s.Dom&starBit == 0 || s.Dow&starBit == 0 {
		...
		for i := 0; i <= max; i++ {
			...
			if i == 0 {
				tChk = time.Date(tChk.Year(), tChk.Month(), tChk.Day(), 0, 0, 0, 0, loc)
			}
			if i != max {
				tChk = tChk.AddDate(0, 0, 1)
				if tChk.Hour() != 0 {
